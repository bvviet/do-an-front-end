  ...
  const getDetail = async (id: string) => {
    try {
      const { data } = await axios.get(
        "http://127.0.0.1:8000/api/voucher/" + id,
      );
    ...
    <div className="mx-auto h-fit max-w-screen-xl rounded-lg bg-white px-4 py-16 sm:px-6 lg:px-8">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="">
        <div className="grid grid-cols-2 gap-6">
              ...
              {...register("discount_value", {
                ...
                validate: {
                  minValue: (value) => {
                    ...
                    return (
                      numValue >= 1 ||
                      "Giảm giá phải tối thiểu lớn hơn hoặc bằng 1%"
                    );
                  },
                  maxValue: (value) => {
                    const numValue = parseFloat(value); // Chuyển giá trị string thành number
          ...
          <div className="relative flex">
            ...
            {(voucher?.used_count ?? 0) > 0 && (
              ...
              >
                <span className="absolute left-full top-0 -translate-x-1/2 -translate-y-1/2 transform text-red-500">
        ...
        <div className="mt-6 flex justify-end">
          <button
            type="submit"
            className="flex items-center justify-center gap-2 rounded-xl bg-[#FFD44D] px-12 py-4 font-manrope text-[14px] leading-[166.667%] text-black disabled:bg-[#FFD44D] disabled:opacity-50 max-lg:text-[12px]"
